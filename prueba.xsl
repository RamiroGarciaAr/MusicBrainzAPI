...
* Life-Span: <xsl:value-of select="life-span/begin"/>-<xsl:template match="life-span/ended"><xsl:if test="true"><xsl:value-of select="life-span/end"/></xsl:if><xsl:if test="false">present</xsl:if></xsl:template>
=== Recordings
</xsl:template>
</xsl:stylesheet>